...
Discord Developer Portal → 해당 애플리케이션 → **OAuth2** 탭 → **Redirects** 에 아래 두 URL을
정확히 추가해야 로그인이 동작합니다 (등록 전까지는 `redirect_uri` 불일치로 로그인 실패):

```
https://discordfrauddb.krl.kr/api/auth/discord/callback
https://discordfrauddb.krl.kr/api/auth/discord-user/callback
```

(첫 번째는 관리자 웹 로그인용, 두 번째는 일반 제보자/서버 인증 신청자용 — `/report`, `/certify`에서 사용)

그 외 채워두면 좋은 값 (`.env`, 없어도 봇/웹은 동작하지만 기능이 제한됨):
- `DISCORD_GUILD_ID` — **우리 서버(허브)** ID. 사기 DB 관리 명령어(`/신고` `/검색` `/사기꾼추가` `/패널설치` `/운영진설정` 등)는
  이 서버에서만 보이고 동작함 (길드 커맨드 등록 + 실행 시점 재검증 이중 방어). 안전서버 인증 명령어(`/안전서버인증신청` 등)는
  전역 등록되어 봇이 초대된 모든 다른 서버에서 사용 가능하지만, 반대로 이 허브 서버 안에서는 실행이 막혀 있음
  (허브 자신은 인증 대상이 아니므로).
- `DISCORD_LOG_CHANNEL_ID` — 신규 제보/등록/수정/삭제/이의제기/인증 로그가 올라갈 채널
- `DISCORD_REPORT_CHANNEL_ID` — 자동 정보 인식(메시지 스캔)을 적용할 채널 (예: #사기제보)
- `DISCORD_EVIDENCE_CHANNEL_ID` — `/신고` 제출 후 증거 파일을 업로드하는 스레드가 생성될 채널
...
- 위 세 명령어는 **전역 등록**되어 봇이 초대된 모든 서버에서 쓸 수 있지만, **우리 서버(허브)** 안에서
  신청/재인증은 실행되지 않도록 막혀 있음 (허브 자신은 인증 대상이 아니므로) — `/인증정보`만 예외적으로 어디서나 조회 가능
- 웹에서도 신청 가능: `/certify` (Discord 로그인 후 내가 관리자 권한을 가진 서버 목록을 불러와 바로 신청),
  인증서버 검색은 `/servers` 또는 웹 홈 화면에서
...
- `/패널설치 종류:검색|제보|이의제기|통계|안내|안전서버|전체` — 현재 채널에 사용자용 패널 임베드 설치
  (채널마다 원하는 패널만 골라 설치 가능 — 여러 채널에 여러 임베드 구성 가능). 이 명령어 자체가 허브 서버 전용.
  - 현재 서버에는 이미 배치되어 있음: #사기꾼-검색(검색), #사기꾼-신고(제보), #문의하기(이의제기),
    #이용약관(안내), #관리자-채팅(통계), #추가된-사기꾼(검증완료 사건 자동 게시 - 버튼 없는 라이브 피드),
    #안전서버(봇 초대 링크 + 웹 신청 링크 + 🔍 인증서버 검색 버튼 - 신청/재인증 버튼은 없음, 허브에선 무의미하므로)
...
### 웹 (https://discordfrauddb.krl.kr)
- `/`, `/search`, `/case/[사건번호]` — 공개 검색 (마스킹, 로그인 불필요)
- `/report` — 사기 제보. **Discord 로그인 필요** (허위 제보 방지, 관리자 화이트리스트 없이 아무 Discord 계정이나 가능)
- `/server/[인증번호]`, `/badge/[인증번호]` — 안전서버 인증 공개 페이지 / 동적 배지 이미지 (로그인 불필요)
- `/servers` — 인증서버 검색 (서버명/인증번호)
- `/certify` — 내 서버 인증 신청. Discord 로그인(guilds 스코프) 후 내가 관리 권한을 가진 서버 목록에서 바로 신청
- `/admin/login` — Discord OAuth 로그인 (Owner/Admin/Manager 화이트리스트, 관리자 전용)
